...
class CDataLoaderMNIST(CDataLoader):
    ...
    def __init__(self):
        ...
        with CDataLoaderMNIST.__lock:
            ...
            if (
                not fm.file_exist(self.train_data_path)
                or md5(self.train_data_path) != TRAIN_DATA_MD5
            ):
                self._get_data(
                    TRAIN_DATA_FILE, MNIST_PATH, self.train_data_path, TRAIN_DATA_MD5
                )
            if (
                not fm.file_exist(self.train_labels_path)
                or md5(self.train_labels_path) != TRAIN_LABELS_MD5
            ):
                self._get_data(
                    TRAIN_LABELS_FILE,
                    MNIST_PATH,
            ...
            if (
                not fm.file_exist(self.test_data_path)
                or md5(self.test_data_path) != TEST_DATA_MD5
            ):
                self._get_data(
                    TEST_DATA_FILE, MNIST_PATH, self.test_data_path, TEST_DATA_MD5
                )
            if (
                not fm.file_exist(self.test_labels_path)
                or md5(self.test_labels_path) != TEST_LABELS_MD5
            ):
                self._get_data(
                    TEST_LABELS_FILE, MNIST_PATH, self.test_labels_path, TEST_LABELS_MD5
                )
    ...
    def load(self, ds, digits=tuple(range(0, 10)), num_samples=None):
        ...
        self.logger.info("Loading MNIST {:} dataset from {:}...".format(ds, MNIST_PATH))
        ...
        if magic_nr != 2049:
            raise ValueError(
                "Magic number mismatch, expected 2049," "got {}".format(magic_nr)
            )
        ...
        if magic_nr != 2051:
            raise ValueError(
                "Magic number mismatch, expected 2051," "got {}".format(magic_nr)
            )
        ...
        if num_samples is not None:
            ...
            n_samples_class = {e: n_samples_class[e_i] for e_i, e in enumerate(digits)}
    ...
    def _get_data(self, file_name, dl_folder, output_path, md5sum):
        ...
        fh = dl_file_gitlab(MODEL_ZOO_REPO_URL, MNIST_REPO_PATH + file_name, dl_folder)
