...
class CDataLoaderImgFolders(CDataLoader):
    ...
    def load(
        self, ds_path, img_format, label_re=None, label_dtype=None, load_data=True
    ):
    ...
    def _explore_dir(
        self, dir_path, img_w, img_h, img_c, img_ext, label_re=None, load_data=True
    ):
        ...
        leaf = not any(fm.folder_exist(fm.join(dir_path, item)) for item in items_list)
        ...
        for subdir in items_list:
            ...
            patterns = (
                patterns.append(patterns_new, axis=0)
                if patterns is not None
                else patterns_new
            )
            labels = labels.append(labels_new) if labels is not None else labels_new
    ...
    def _load_files(
        ...
        for file_name in files_list:
            ...
            if fm.splitext(file_name)[1].lower() == img_ext:
                ...
                labels = labels.append(c_id) if labels is not None else CArray(c_id)

                self.logger.debug(
                    "{:} has been loaded..." "".format(fm.join(dir_path, file_name))
                )
