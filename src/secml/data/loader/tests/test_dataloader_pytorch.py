...
class TestCDataLoaderPytorch(CUnitTest):
    ...
    def _dataset_creation_blobs(self):
        ...
        dataset = CDLRandom(
            n_samples=self.n_samples_tr + self.n_samples_ts,
        ...
        splitter = CTrainTestSplit(
            train_size=self.n_samples_tr, test_size=self.n_samples_ts, random_state=0
        )
        ...
        self._tr_loader = CDataLoaderPyTorch(
            self.tr.X, self.tr.Y, self.batch_size, shuffle=True, transform=None
        ).get_loader()

        self._ts_loader = CDataLoaderPyTorch(
            self.ts.X, self.ts.Y, self.batch_size, shuffle=False, transform=None
        ).get_loader()
    ...
    def _dataset_creation_mnist(self):
        ...
        self._tr_loader = CDataLoaderPyTorch(
            self.tr.X, self.tr.Y, self.batch_size, shuffle=True, transform=transform
        ).get_loader()

        self._ts_loader = CDataLoaderPyTorch(
            self.ts.X, self.ts.Y, self.batch_size, shuffle=False, transform=transform
        ).get_loader()
