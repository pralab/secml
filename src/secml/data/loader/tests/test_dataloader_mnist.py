...
class TestCDataLoaderMNIST(CUnitTest):
    ...
    def test_load(self):
        ...
        self.logger.info("Loading {:} training set samples".format(tr.num_samples))

        self.assertEqual(tr.num_samples, 18112)

        ts = CDataLoaderMNIST().load("testing", digits=digits)

        self.logger.info("Loading {:} test set samples".format(ts.num_samples))
        ...
        tr = CDataLoaderMNIST().load("training", digits=digits, num_samples=n_tr)

        self.logger.info("Loading {:} training set samples".format(tr.num_samples))

        self.assertEqual(tr.num_samples, n_tr)

        ts = CDataLoaderMNIST().load("testing", digits=digits, num_samples=n_ts)
