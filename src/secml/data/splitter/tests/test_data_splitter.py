...
class TestCDataSplitter(CUnitTest):
    ...
    def test_kfold(self):
        ...
        kf = CDataSplitterKFold(num_folds=2, random_state=5000).compute_indices(ds)
        ...
        for fold_idx in range(kf.num_folds):
            self.logger.info(
                "{:} fold: \nTR {:} \nTS {:}"
                "".format(fold_idx, kf.tr_idx[fold_idx], kf.ts_idx[fold_idx])
            )
            self.assert_array_equal(tr_idx_expected[fold_idx], kf.tr_idx[fold_idx])
            self.assert_array_equal(ts_idx_expected[fold_idx], kf.ts_idx[fold_idx])

    def test_labelkfold(self):

        ds = CDLRandom(
            n_classes=3, n_samples=10, n_informative=3, random_state=0
        ).load()
    ...
    def test_openworldkfold(self):
        ...
        ds = CDLRandom(
            n_classes=3, n_samples=14, n_informative=3, random_state=0
        ).load()

        self.logger.info("Testing Open World K-Fold")
        kf = CDataSplitterOpenWorldKFold(
            num_folds=2, n_train_samples=4, random_state=5000
        ).compute_indices(ds)

        tr_idx_expected = [CArray([0, 4, 8, 12]), CArray([1, 3, 9, 13])]
        ts_idx_expected = [
            CArray([1, 2, 3, 5, 6, 7, 9, 10, 11, 13]),
    ...
    def test_openworldkfold_tr_class_skip(self):
        ...
        kf = CDataSplitterOpenWorldKFold(
            num_folds=25, n_train_samples=2, random_state=5000
        ).compute_indices(ds)
    ...
    def test_shuffle(self):
        ...
        kf = CDataSplitterShuffle(
            num_folds=2, train_size=0.2, random_state=5000
        ).compute_indices(ds)

        tr_idx_expected = [CArray([1, 2]), CArray([9, 3])]
        ts_idx_expected = [
            CArray([6, 4, 7, 0, 3, 9, 5, 8]),
