...
class LogitsLoss:
    ...
    def _adv_objective_function(self, x):
        ...
        second_best_labels = torch.argmax(
            y_target * torch.min(z_predicted) + z_predicted * (1 - y_target), dim=1
        )
        z_other = z_predicted[
            torch.arange(z_predicted.size(0)).long(), second_best_labels
        ]

        if self.y_target is not None:
            # if targeted, optimize for making the target class most likely
            loss = torch.max(
                z_other - z_target + self.confidence,
                torch.zeros(x.shape[0], dtype=x.dtype),
            )
        else:
            # if untargeted, optimize for making any other class most likely
            loss = torch.max(
                z_target - z_other + self.confidence,
                torch.zeros(x.shape[0], dtype=x.dtype),
            )
