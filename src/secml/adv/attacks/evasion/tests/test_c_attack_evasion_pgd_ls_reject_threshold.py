...
class TestCAttackEvasionPGDLSRejectThreshold(CAttackEvasionTestCases):
    ...
    def _dataset_creation(self):
        ...
        # Data normalization
        self.normalizer = CNormalizerMinMax(feature_range=(self.lb, self.ub))
    ...
    def _test_evasion_multiclass(self, expected_x):
        ...
        y_pred, score = self.multiclass.predict(x_opt, return_decision_function=True)
        ...
        self.logger.info(
            "Number of gradient function evaluations: {:}".format(eva.grad_eval)
        )
        ...
        if self.y_target:
            ...
            s_ytarget_xopt = self.multiclass.decision_function(x_opt, self.y_target)
