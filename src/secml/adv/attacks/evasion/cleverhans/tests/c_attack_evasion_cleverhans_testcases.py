...
class CAttackEvasionCleverhansTestCases(CAttackEvasionTestCases):
    ...
    def _test_confidence(self, x0, y0, x_opt, clf, y_target):
        ...
        """
        init_pred, init_score = clf.predict(x0, return_decision_function=True)
        final_pred, final_score = clf.predict(x_opt, return_decision_function=True)

        if y_target is not None:
            self.assertGreater(
                final_score[:, y_target].item(), init_score[:, y_target].item()
            )
