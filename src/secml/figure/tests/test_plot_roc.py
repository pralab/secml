...
class TestCRoc(CUnitTest):
    ...
    def setUp(self):
        ...
        self.ds_loader = CDLRandom(
            n_features=1000, n_redundant=200, n_informative=250, n_clusters_per_class=2
        )
        ...
        _, self.s1 = self.svm.predict(self.ds1.X, return_decision_function=True)
        _, self.s2 = self.svm.predict(self.ds2.X, return_decision_function=True)
    ...
    def test_compare_sklearn(self):
        ...
        for i, (train, test) in enumerate(cv.split(X, y)):
            ...
            roc_auc = auc(fpr, tpr)
            roc_fig.sp.plot(
                fpr, tpr, linewidth=1, label="ROC fold %d (area = %0.2f)" % (i, roc_auc)
            )

        roc_fig.sp.plot([0, 1], [0, 1], "--", color=(0.6, 0.6, 0.6), label="Luck")
        ...
        roc_fig.sp.plot([0, 100], [0, 100], "--", color=(0.6, 0.6, 0.6), label="Luck")
        ...
        roc_fig.sp.plot_roc_mean(
            ...
            logx=False,
            style="go-",
            label="Mean ROC (area = %0.2f)" % (auc(fp.tondarray(), tp.tondarray())),
        )
