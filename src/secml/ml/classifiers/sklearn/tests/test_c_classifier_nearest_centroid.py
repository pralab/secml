...
class TestCClassifierNearestCentroid(CClassifierTestCases):
    ...
    def test_plot(self):
        ...
        fig.savefig(
            fm.join(
                fm.abspath(__file__), "figs", "test_c_classifier_nearest_centroid.pdf"
            )
        )
    ...
    def test_preprocess(self):
        ...
        self._test_preprocess(
            ds, self.nc, ["min-max", "mean-std"], [{"feature_range": (-1, 1)}, {}]
        )

        # Mixed linear/nonlinear transformations
        self._test_preprocess(ds, self.nc, ["pca", "unit-norm"], [{}, {}])
