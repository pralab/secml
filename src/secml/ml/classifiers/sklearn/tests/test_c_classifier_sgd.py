...
class TestCClassifierSGD(CClassifierTestCases):
    ...
    def setUp(self):
        ...
        kernel_types = (None, CKernelLinear(), CKernelRBF(), CKernelPoly(degree=3))
        self.sgds = [
            CClassifierSGD(
        ...
        self.logger.info("Testing SGD with kernel functions: %s", str(kernel_types))
    ...
    def test_draw(self):
        ...
        dataset = CDLRandom(
            n_features=2, n_redundant=1, n_informative=1, n_clusters_per_class=1
        ).load()
        ...
        fig.sp.plot_fun(svm.decision_function, grid_limits=dataset.get_bounds(), y=1)
        fig.sp.title("SVM")
        ...
        # Plot objective function
        fig.sp.plot_fun(
            self.sgds[0].decision_function, grid_limits=dataset.get_bounds(), y=1
        )
        fig.sp.title("SGD Classifier")

        fig.savefig(fm.join(fm.abspath(__file__), "figs", "test_c_classifier_sgd1.pdf"))
    ...
    def test_performance(self):
        ...
        for sgd in self.sgds:
            ...
            acc_sgd = CMetric.create("f1").performance_score(self.dataset.Y, label_sgd)
    ...
    def test_margin(self):
        ...
        fig.sp.scatter(
            dataset.X[:, 0].ravel(), dataset.X[:, 1].ravel(), c=dataset.Y, s=40
        )
