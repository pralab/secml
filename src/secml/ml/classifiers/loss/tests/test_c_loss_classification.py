...
class TestCLossClassification(CUnitTest):
    ...
    def test_one_at_zero(self):
        ...
        for loss_id in ("hinge", "hinge-squared", "square", "log"):
            ...
            self.assertEqual(CArray([1.0]), loss_class.loss(CArray([1]), CArray([0])))
    ...
    def test_draw(self):
        ...
        fig.sp.plot(x, CArray([1 if i <= 0 else 0 for i in x]), label="0-1 indicator")
    ...
    def test_grad(self):
        ...
        for loss_id in ("hinge", "hinge-squared", "square", "log"):
            ...
            check_grad_val = CFunction(_loss_wrapper, _dloss_wrapper).check_grad(
