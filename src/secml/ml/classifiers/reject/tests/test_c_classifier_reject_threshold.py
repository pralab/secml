...
class TestCClassifierRejectThreshold(CClassifierRejectTestCases):
    ...
    def setUp(self):
        ...
        self.dataset = CDLRandomBlobs(
            n_features=2, n_samples=100, centers=2, cluster_std=2.0, random_state=0
        ).load()

        self.logger.info("Testing classifier creation ")
        self.clf_norej = CClassifierSGD(
    ...
    def test_reject(self):
        ...
        y_pred, score_pred = clf.predict(self.dataset.X, return_decision_function=True)
        ...
        acc = CMetric.create("accuracy").performance_score(y_pred, self.dataset.Y)
        self.logger.info("Accuracy no rejection: {:}".format(acc))

        rej_acc = CMetric.create("accuracy").performance_score(
            y_pred_reject[y_pred_reject != -1], self.dataset.Y[y_pred_reject != -1]
        )
    ...
    def test_gradient(self):
        ...
        grads_d = self._test_gradient_numerical(clf, ds.X[i, :], extra_classes=[-1])

        self.logger.info("Testing with sparse data...")
        ds = self.dataset.tosparse()
        clf = self.clf.fit(ds.X, ds.Y)

        grads_s = self._test_gradient_numerical(clf, ds.X[i, :], extra_classes=[-1])
    ...
    def test_preprocess(self):
        ...
        self._test_preprocess(self.dataset, self.clf, ["pca", "unit-norm"], [{}, {}])
    ...
    def test_draw(self):
        ...
        fig.sp.plot_ds(self.dataset[y == -1, :], colors=["k", "k"], markersize=12)
