...
class TestCClassifierMultiOVO(CClassifierTestCases):
    ...
    def test_predict_withsvm(self):
        ...
        multiclass = CClassifierMulticlassOVO(
            classifier=CClassifierSVM, class_weight="balanced", n_jobs=2
        )
        ...
        acc = CMetric.create("accuracy").performance_score(self.dataset.Y, class_pred)
    ...
    def test_set(self):
        ...
        multiclass = CClassifierMulticlassOVO(
            classifier=CClassifierSVM, C=1, kernel=CKernelRBF()
        )
        ...
        for i in range(1, multiclass.num_classifiers):
            self.assertNotEqual(multiclass._binary_classifiers[i].kernel.gamma, 300.0)
    ...
    def test_apply_method(self):
        ...
        multiclass.fit(self.dataset.X, self.dataset.Y)
        multiclass.apply_method(CClassifierSVM.set, param_name="C", param_value=150)
    ...
    def test_normalization(self):
        ...
        multi_nonorm = CClassifierMulticlassOVO(
            classifier=CClassifierSVM, class_weight="balanced"
        )
        multi_nonorm.fit(ds_norm_x, self.dataset.Y)
        pred_y_nonorm = multi_nonorm.predict(ds_norm_x)

        multi = CClassifierMulticlassOVO(
            classifier=CClassifierSVM, class_weight="balanced", preprocess="min-max"
        )
        ...
        self.logger.info("Predictions with external norm:\n{:}".format(pred_y_nonorm))
    ...
    def test_plot_decision_function(self):
        ...
        y_pred, score_pred = multiclass.predict(ds.X, return_decision_function=True)
        ...
        fig.sp.title(
            "{:} ({:})".format(
                multiclass.__class__.__name__, multiclass.classifier.__name__
            )
        )
        ...
        for c_idx, c in enumerate(ds.classes):
            ...
            plot_hyperplane(
                fig,
                multiclass._binary_classifiers[c_idx],
                x_bounds[0],
                x_bounds[1],
                styles[c_idx],
                "Boundary\nfor class {:}".format(c),
            ...
            fig.sp.scatter(
                ds.X[ds.Y == c, 0], ds.X[ds.Y == c, 1], s=40, c=styles[c_idx][0]
            )
            fig.sp.scatter(
                ds.X[y_pred == c, 0],
                ds.X[y_pred == c, 1],
        ...
        fig.sp.plot_decision_regions(
            multiclass, n_grid_points=100, grid_limits=ds.get_bounds(offset=5)
        )

        fig.sp.xlim(x_bounds[0] - 0.5 * x_bounds[1], x_bounds[1] + 0.5 * x_bounds[1])
        fig.sp.ylim(y_bounds[0] - 0.5 * y_bounds[1], y_bounds[1] + 0.5 * y_bounds[1])
    ...
    def test_gradient(self):
        ...
        for i in range(multiclass.n_classes):
            ...
            for j in range(multiclass.num_classifiers):
                ...
                if idx_pos == i:
                    w_bin = CArray([1, 0])
                    grad_pos = multiclass._binary_classifiers[j].gradient(
                        pattern, w_bin
                    )
                    ovo_grad_pos += grad_pos
                if idx_neg == i:
                    w_bin = CArray([0, 1])
                    grad_neg = multiclass._binary_classifiers[j].gradient(
                        pattern, w_bin
                    )
    ...
    def test_multiclass_gradient(self):
        ...
        def grad_f_x(p):
            w = CArray.ones(shape=multiclass.n_classes) / (div * multiclass.n_classes)
