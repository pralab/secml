...
class TestCPerfEvaluatorMulticlass(CUnitTest):
    ...
    def setUp(self):
        ...
        self.tr = CDLRandom(
            n_classes=4, n_clusters_per_class=1, random_state=50000
        ).load()
        self.ts = CDLRandom(
            n_classes=4, n_clusters_per_class=1, random_state=10000
        ).load()
    ...
    def _run_multiclass(self, multiclass, xval_params, expected_best):
        ...
        for clf_idx, clf in enumerate(multiclass._binary_classifiers):
            self.assertEqual(clf.C, expected_best["C"][clf_idx])
            self.assertEqual(clf.kernel.gamma, expected_best["kernel.gamma"][clf_idx])
        ...
        for clf_idx, clf in enumerate(multiclass._binary_classifiers):
            for param in best_params:
                self.assertEqual(clf.get_params()[param], best_params[param][clf_idx])
    ...
    def test_params_multiclass(self):
        ...
        expected = {"C": [1.0, 1.0, 10.0, 10.0], "kernel.gamma": [0.1, 0.1, 0.1, 0.1]}
        ...
        expected = {"C": [100, 10, 10, 1], "kernel.gamma": [0.1, 0.1, 0.1, 0.1]}
