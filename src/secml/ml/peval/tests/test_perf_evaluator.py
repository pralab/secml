...
class TestCPerfEvaluator(CUnitTest):
    ...
    def test_parameters_setting(self):
        ...
        self.svm.estimate_parameters(
            self.training_dataset, xval_parameters, xval_splitter, "accuracy"
        )

        self.logger.info(
            "SVM has now the following parameters: {:}".format(self.svm.get_params())
        )
        ...
        for param in xval_parameters:
            self.logger.info("Best '{:}' is: {:}".format(param, best_params[param]))
        ...
        for comb in range(len(parameters_combination)):
            ...
            for f in range(num_xval_fold):
                ...
                this_fold_accuracy = skm.accuracy_score(
                    self.training_dataset[xval_splitter.ts_idx[f], :].Y.get_data(),
                    this_fold_predicted.get_data(),
                )
                this_fold_score.append(this_fold_accuracy)

            par_comb_score[comb] = np.mean(this_fold_score)
            self.logger.info("this fold mean: {:}".format(par_comb_score[comb]))

        max_combination_score = par_comb_score.max()
        better_param_comb = parameters_combination[par_comb_score.argmax()]
        self.logger.info(
            "max combination score founded here: {:}".format(max_combination_score)
        )
        self.logger.info("max comb score founded during xval {:}".format(best_score))
    ...
    def _run_multiclass(self, tr, multiclass, xval_params, expected_best):
        ...
        xval_splitter = CDataSplitter.create("kfold", num_folds=3, random_state=50000)
    ...
    def test_params_multiclass(self):
        ...
        # Create dummy dataset (we want a test different from train)
        tr = CDLRandom(n_classes=4, n_clusters_per_class=1, random_state=50000).load()

        kernel = CKernel.create("rbf")
        multiclass = CClassifierMulticlassOVA(CClassifierSVM, C=1, kernel=kernel)
