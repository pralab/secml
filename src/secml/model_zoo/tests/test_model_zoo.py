...
class TestModelZoo(CUnitTest):
    ...
    def setUpClass(cls):
        ...
        cls.test_models_def = fm.join(fm.abspath(__file__), "models_dict_test.json")

        # Test model's definition
        cls.test_model_id = "_test_model"
        cls.test_model = fm.join(fm.abspath(__file__), "_test_model_clf.py")
        cls.test_model_state = fm.join(fm.abspath(__file__), "_test_model-clf.gz")
    ...
    def _mock_requests(self, m, defs_url=None, model_url=None, state_url=None):
        ...
        if defs_url:  # Mocking models definitions
            ...
            with open(self.test_models_def) as fdefs:
                m.get(defs_url, text=fdefs.read(), headers=self.api_defs_headers)

        if model_url:  # Mocking model
            self.logger.info("Mocking `{:}`".format(model_url))
            with open(self.test_model) as fmodel:
                m.get(model_url, text=fmodel.read(), headers=self.api_model_headers)
    ...
    def test_load_model_vers(self):
        ...
        self._test_load_model(
            defs_url=self.api_url_defs_vers,
    ...
    def test_load_model_master(self):
        ...
        self._test_load_model(
            defs_url=self.api_url_defs_master,
    ...
    def test_load_model_fail(self):
        ...
        with self.assertRaises(KeyError):
            with requests_mock.Mocker() as m:
                self._mock_requests(
                    m, defs_url=self.api_url_defs_vers, model_url=None, state_url=None
                )
                load_model("svm-test")
