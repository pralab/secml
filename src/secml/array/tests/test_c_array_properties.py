...
class TestCArrayProperties(CArrayTestCases):
    ...
    def test_non_zero_indices(self):
        ...
        def non_zero_indices(self, structure_name, matrix, row_vector, column_vector):
            ...
            self.assertTrue(all(isinstance(elem, list) for elem in matrix.nnz_indices))

        non_zero_indices(
            self, "sparse", self.array_sparse, self.row_sparse, self.column_sparse
        )
        non_zero_indices(
            self, "dense", self.array_dense, self.row_sparse, self.column_dense
        )
