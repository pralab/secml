...
class TestCArrayIndexing(CArrayTestCases):
    ...
    def test_getter(self):
        ...
        def test_selectors(input_array, selector_list, target_list):
            ...
            for selector_idx, selector in enumerate(selector_list):
                ...
                self.assertFalse(
                    CArray(selection != target_list[selector_idx]).any(),
                ...
                if isinstance(target_list[selector_idx], CArray):
                    ...
                    if selection.issparse:
                        ...
                        self.assertEqual(
                            target_list[selector_idx].shape, selection.shape
                        )
                    self.assertEqual(target_list[selector_idx].dtype, selection.dtype)
                else:
                    self.assertIsInstance(selection, type(target_list[selector_idx]))
        ...
        for array in arrays_list:
            ...
            selectors = (
                selectors_a
                + [(x, y) for x in selectors_row for y in selectors_col]
                + selectors_b
            )
    ...
    def test_setter(self):
        ...
        for array in arrays_list:
            ...
            targets_a = 2 * [CArray([[1, 2, 0], [2, 4, 6], [0, 6, 10]])] + 7 * [
                CArray([[1, 2, 0], [2, 4, 6], [0, 10, 20]])
            ]
            targets_b = 2 * [CArray([[1, 2, 0], [2, 4, 10], [0, 6, 20]])] + 7 * [
                CArray([[1, 2, 0], [2, 10, 20], [0, 30, 40]])
            ]
        ...
        for array in arrays_list:
            ...
            assignments_b = (
                [0] + [10, 10] + 2 * [CArray([[10, 20]])] + 3 * [CArray([10, 20])]
            )
            ...
            if array.ndim == 1:
                ...
                targets = (
                    4 * [targets_a.atleast_2d()]
                    + 4 * [targets_b.atleast_2d()]
                    + 12
                    * (
                        [CArray([[4, 0, 6]])]
                        + 2 * [targets_c.atleast_2d()]
                        + 5 * [targets_d.atleast_2d()]
                    )
                )
        ...
        for array in arrays_list:
            ...
            if array.ndim == 1:
                ...
                targets = (
                    ...
                    + 3 * [targets_b.atleast_2d()]
                    + [targets_c.atleast_2d()]
                    + 2 * [targets_a.atleast_2d()]
                )
