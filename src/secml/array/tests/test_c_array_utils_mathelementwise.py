...
class TestCArrayUtilsMathElementWise(CArrayTestCases):
    ...
    def test_sqrt(self):
        ...
        _check_sqrt(self.row_flat_dense, CArray([2.0, 0.0, 2.4495]))
        _check_sqrt(CArray([4.0, 0.0, -3.0]), CArray([2.0, 0.0, nan]))
        _check_sqrt(self.row_dense, CArray([[2.0, 0.0, 2.4495]]))
        _check_sqrt(self.row_sparse, CArray([[2.0, 0.0, 2.4495]], tosparse=True))
        _check_sqrt(CArray([[4.0, 0.0, -3.0]]), CArray([[2.0, 0.0, nan]]))
        _check_sqrt(
            CArray([4.0, 0.0, -3.0], tosparse=True),
            CArray([[2.0, 0.0, nan]], tosparse=True),
        )
        _check_sqrt(self.column_dense, CArray([[2.0], [0.0], [2.4495]]))
        _check_sqrt(self.column_sparse, CArray([[2.0], [0.0], [2.4495]], tosparse=True))
        _check_sqrt(self.single_flat_dense, CArray([2.0]))
        _check_sqrt(self.single_dense, CArray([[2.0]]))
        _check_sqrt(self.single_sparse, CArray([[2.0]], tosparse=True))
    ...
    def test_cos(self):
        ...
        _check_cos(
            ...
            CArray(
                [
                    [0.5403, 1.0, 1.0, 0.2837],
                    [-0.4161, -0.6536, 1.0, 1.0],
                    [-0.9900, 0.9602, 1.0, 1.0],
                ]
            ),
        )
        _check_cos(self.row_flat_dense, CArray([-0.6536, 1.0, 0.9602]))
        _check_cos(self.row_dense, CArray([[-0.6536, 1.0, 0.9602]]))
        _check_cos(self.column_dense, CArray([[-0.6536], [1.0], [0.9602]]))
    ...
    def test_exp(self):
        ...
        _check_exp(
            ...
            CArray(
                [
                    [2.7183, 1.0, 1.0, 148.4132],
                    [7.3891, 54.5982, 1.0, 1.0],
                    [20.0855, 403.4288, 1.0, 1.0],
                ]
            ),
        )
        _check_exp(self.row_flat_dense, CArray([54.5982, 1.0, 403.4288]))
        _check_exp(self.row_dense, CArray([[54.5982, 1.0, 403.4288]]))
        _check_exp(self.column_dense, CArray([[54.5982], [1.0], [403.4288]]))
    ...
    def test_log10(self):
        ...
        _check_log10(self.column_dense, CArray([[0.6021], [-inf], [0.7782]]))
