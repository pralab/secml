...
class TestCArrayInit(CArrayTestCases):
            ...
            for tosparse in [False, True]:
                ...
                if is_list_of_lists(totest_elem):
                    if not is_list_of_lists(totest_elem[0]):
                        self.assertEqual(init_array.shape[0], len(totest_elem))
                        self.assertEqual(init_array.shape[1], len(totest_elem[0]))
                        ...
                        self.assertEqual(init_array.shape[0], len(totest_elem))
                        self.assertEqual(init_array.shape[1], sum(in_shape[1:]))

                elif is_list(totest_elem):
                    if init_array.issparse is True:
                        self.assertEqual(init_array.shape[1], len(totest_elem))
                    elif init_array.isdense is True:
                        self.assertTrue(init_array.ndim == 1)
                        self.assertEqual(init_array.shape[0], len(totest_elem))
                    self.assertEqual(init_array.input_shape, (len(totest_elem),))
        ...
        for a in arrays:
            for sparse in (False, True):
                ...
                for shape in [size, (size,), (1, size), (size, 1)]:
                    ...
                    self.logger.info(
                        "Expected 'shape' {:}, got {:}".format(shape, out_res.shape)
                    )
    ...
    def test_init_empty(self):
        ...
        for test_case in (False,):
            self.logger.info("Testing flat empty, tosparse: {:}".format(test_case))
        ...
        for test_case in (False, True):
            self.logger.info("Testing 2D empty, tosparse: {:}".format(test_case))
            array_empty = CArray(empty_init, tosparse=test_case)
            self.assertEqual(array_empty.size, 0)
            self.assertEqual(array_empty.shape, (1, 0))
            self.assertEqual(array_empty.ndim, 2)
    ...
    def test_input_shape(self):
        ...
        array_s = CArray([[[2, 3], [22, 33]], [[4, 5], [44, 55]]], tosparse=True)
        ref_shape = (2, 2, 2)
