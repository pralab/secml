...
class TestDownloadUtils(CUnitTest):
    ...
    def test_dlfile(self, m):
        ...
        self.assertTrue("File is not available (error code 404)" in str(e.exception))
    ...
    def test_dlfile_headers(self, m):
        ...
        out_file = dl_file(url, self.tempdir, headers={"TOKEN": "test", "HEADER2": "2"})
    ...
    def test_dlfile_content_disposition(self, m):
        ...
        def _test_dlfile(cont_disp, fn="test.txt"):
            ...
            m.get(
                url,
                text=file_content,
                headers={"Content-Length": "4", "Content-Disposition": cont_disp},
            )
            out_file = dl_file(url, self.tempdir)
    ...
    def test_dlfile_gitlab(self, m):
        ...
        url = api_url.format(
            parse.quote(repo, safe=""), parse.quote(file, safe=""), branch
        )
        file_content = "resp"
        ...
        m.get(
            url,
            text=file_content,
            headers={"Content-Length": "4", "Content-Disposition": disp},
        )
        ...
        dl_file_gitlab(repo + "/", file, self.tempdir, branch=branch)
        dl_file_gitlab("gitlab.com/" + repo, file, self.tempdir, branch=branch)
        dl_file_gitlab("gitlab.com/" + repo + "/", file, self.tempdir, branch=branch)
        dl_file_gitlab("https://gitlab.com/" + repo, file, self.tempdir, branch=branch)
        dl_file_gitlab("http://gitlab.com/" + repo, file, self.tempdir, branch=branch)
        dl_file_gitlab(repo, "/" + file, self.tempdir, branch=branch)
