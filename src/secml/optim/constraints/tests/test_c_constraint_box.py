...
class TestConstraintBox(CConstraintTestCases):
    ...
    def test_is_active(self):
        ...
        self._test_is_active(self.c, self.p1_inside, self.p2_outside, self.p3_on)

        # Test for sparse arrays
        self._test_is_active(
            self.c,
            self.p1_inside.tosparse(),
            self.p2_outside.tosparse(),
            self.p3_on.tosparse(),
        )
    ...
    def test_is_violated(self):
        ...
        self._test_is_violated(self.c, self.p1_inside, self.p2_outside, self.p3_on)
        ...
        self._test_is_violated(c, self.p1_inside, self.p2_outside, self.p3_on)
    ...
    def test_constraint(self):
        ...
        self._test_constraint(self.c, self.p1_inside, self.p2_outside, self.p3_on)
    ...
    def test_projection(self):
        ...
        self._test_projection(
            self.c, self.p1_inside, self.p2_outside, self.p3_on, CArray([1.5, 1.5])
        )
        ...
        self._test_projection(
            c,
            CArray([1.0, 1.0, 0.0], tosparse=True),
            CArray([2.0, 2.0, 0.0], tosparse=True),
            CArray([0.0, 1.0, 0.0]).tosparse(),
        ...
        c = CConstraintBox(lb=CArray([0, -inf]), ub=1.5)
        self._test_projection(
            c, self.p1_inside, self.p2_outside, p_out_expected=CArray([1.5, 1.5])
        )

        c = CConstraintBox(lb=CArray([-inf, -0.5]), ub=inf)
        self._test_projection(  # Using [-2, -2] as outside, expect [-2, -0.5]
            c, self.p1_inside, -self.p2_outside, p_out_expected=CArray([-2, -0.5])
        )

    def test_plot(self):
        """Visualize the constraint."""
        # Plotting constraint and "critical" points
        self._test_plot(self.c, self.p1_inside, self.p2_outside, self.p3_on)
