...
class COptimizerTestCases(CUnitTest):
    ...
    def _create_poly(d):
        ...
        int_fun.global_min = lambda: 0.0
        int_fun.global_min_x = lambda: CArray.zeros(
            d,
        )
    ...
    def _plot_optimization(
        solver, x_0, g_min, grid_limits, method=None, vmin=None, vmax=None, label=None
    ):
        ...
        if solver.bounds is not None:  # Plot box constraint
            fig.sp.plot_fun(
                func=lambda x: solver.bounds.constraint(x),
        ...
        if solver.constr is not None:  # Plot distance constraint
            fig.sp.plot_fun(
                func=lambda x: solver.constr.constraint(x),
                plot_background=False,
                n_grid_points=20,
                grid_limits=grid_limits,
                levels=[0],
        ...
        if method is None:
            filename = fm.join(
                test_img_fold_path, solver.class_type + "-" + solver.f.class_type
            )
        else:
            filename = fm.join(
                test_img_fold_path,
    ...
    def _test_constr_bounds(self, opt_class):
        ...
        self.logger.info(
            "Testing minimization of {:} using {:} "
            "and constr {:}".format(
                fun.__class__.__name__, opt.__class__.__name__, opt_params["constr"]
            )
        )
        ...
        self.logger.info(
            "Testing minimization of {:} using {:}, constr {:} "
            "and bounds {:}".format(
        ...
        with self.logger.catch_warnings(record=True) as w:
            min_x = opt.minimize(fun_dict["x0"])
            self.assertIn("is outside of the given bounds", w[0].message.args[0])
