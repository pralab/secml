...
class TestCKernelPolyNumba(CUnitTest):
    """Unit test for CKernelPolyNumba.
    ...
    def setUp(self):
        ...
        self.kernel_numba = CKernelPolyNumba()

        # System should create a kernel \w numba instance if numba is available
        self.assertEqual(CKernel.create('poly').__class__.__name__,
                         CKernelPolyNumba().__class__.__name__)

    def test_similarity(self):
        """Tests similarity function."""
        self.logger.info("Testing POLY kernel - Dense")
        with self.timer():
            simil = self.kernel.k(self.d_dense.X)

        self.logger.info("Testing POLY kernel \w Numba - Dense")
        with self.timer():
        ...
        self.logger.info("Testing POLY kernel - Sparse")
        with self.timer():
        ...
        self.logger.info("Testing POLY kernel \w Numba - Sparse")
        with self.timer():
    ...
    def test_sparser_data(self):
        ...
        self.logger.info("Testing POLY kernel timing on sparser data")
        with self.timer():
            self.kernel.k(self.m1, self.m2)

        self.logger.info("Testing POLY kernel \w Numba timing on sparser data")
        with self.timer():
            self.kernel_numba.k(self.m1, self.m2)

    def test_gradient(self):
        """Tests gradient function."""
        self.logger.info("Testing POLY gradient - Dense")
        with self.timer():
        ...
        self.logger.info("Testing POLY gradient \w Numba - Dense")
        with self.timer():
        ...
        self.logger.info("Testing POLY gradient - Sparse")
        with self.timer():
        ...
        self.logger.info("Testing POLY gradient \w Numba - Sparse")
        with self.timer():
