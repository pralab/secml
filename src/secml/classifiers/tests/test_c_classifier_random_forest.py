...
class TestCClassifierRandomForest(CUnitTest):
    ...
    def test_fun(self):
        ...
        df_scores_0 = self.rnd_forest.discriminant_function(x, label=0)
        self.logger.info(
            "discriminant_function(x, label=0):\n{:}".format(df_scores_0))
        _check_df_scores(df_scores_0, self.dataset.num_samples)

        df_scores_1 = self.rnd_forest.discriminant_function(x, label=1)
        self.logger.info(
            "discriminant_function(x, label=1):\n{:}".format(df_scores_1))
        _check_df_scores(df_scores_1, self.dataset.num_samples)

        df_scores_2 = self.rnd_forest.discriminant_function(x, label=2)
        self.logger.info(
        ...
        ds_priv_scores_0 = self.rnd_forest._discriminant_function(x_norm, label=0)
        ...
        ds_priv_scores_1 = self.rnd_forest._discriminant_function(x_norm, label=1)
        ...
        ds_priv_scores_2 = self.rnd_forest._discriminant_function(x_norm, label=2)
        ...
        labels, scores = self.rnd_forest.classify(x)
        self.logger.info(
        ...
        df_scores_0 = self.rnd_forest.discriminant_function(p, label=0)
        self.logger.info(
            "discriminant_function(p, label=0):\n{:}".format(df_scores_0))
        _check_df_scores(df_scores_0, 1)

        df_scores_1 = self.rnd_forest.discriminant_function(p, label=1)
        self.logger.info(
            "discriminant_function(p, label=1):\n{:}".format(df_scores_1))
        _check_df_scores(df_scores_1, 1)

        df_scores_2 = self.rnd_forest.discriminant_function(p, label=2)
        self.logger.info(
        ...
        labels, scores = self.rnd_forest.classify(p)
        self.logger.info(
            "classify(p):\nlabels: {:}\nscores: {:}".format(labels, scores))
