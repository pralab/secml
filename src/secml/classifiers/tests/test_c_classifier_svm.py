...
class TestCClassifierSVM(CUnitTest):
    ...
    def test_fun(self):
        ...
        for svm in self.svms:
            ...
            df_scores_pos = svm.discriminant_function(
            ...
            df_scores_neg = svm.discriminant_function(
            ...
            ds_priv_scores = svm._discriminant_function(
                self.dataset.X, label=1)
            ...
            df_priv_scores = svm._discriminant_function(
                self.dataset.X[0, :].ravel(), label=1)
            ...
            with self.assertRaises(ValueError):
                svm._discriminant_function(self.dataset.X, label=0)
            with self.assertRaises(ValueError):
                svm._discriminant_function(
                    self.dataset.X[0, :].ravel(), label=0)

            self.logger.info("Testing classify on single point")

            labels, scores = svm.classify(self.dataset.X[0, :].ravel())
